# 實驗 5：Pipe Spray 與 Slab Allocator Lifecycle 觀察

## 1. 實驗目的

本實驗的目的不是重現 kernel exploit，也不是執行 privilege escalation，而是在受控 VM 中觀察一個真實 user-space allocation primitive 對 Linux kernel slab allocator 的影響。
...
for c in /sys/kernel/slab/kmalloc-* /sys/kernel/slab/kmalloc-cg-*; do
  [ -d "$c" ] || continue
  ...
  object_size="$(read_field object_size)"
  objs_per_slab="$(read_field objs_per_slab)"
  order="$(read_field order)"
  objects="$(read_field objects)"
  slabs="$(read_field slabs)"
  partial="$(read_field partial)"
...
```bash
mkdir -p ~/kernel-lab/exp5-pipe-spray
...
```text
1. 部分 kernel objects 的生命週期比 pipe file descriptor 更長
2. allocator 保留 slabs 以便後續重用
3. VM 背景系統活動造成干擾
4. snapshot 時機捕捉到尚未完全回收的狀態
5. per-CPU partial slab 或 SLUB cache 行為造成延遲回收
```
...
```text
在 Ubuntu 6.17 VM 中，建立 20000 pipes 會讓多個 kmalloc cache 的 objects/slabs 明顯增加，尤其是 kmalloc-rnd-05-512、kmalloc-rnd-05-192、kmalloc-rnd-05-128 與 kmalloc-cg-1k。pipes 關閉後，部分 cache 回落，但 allocator 狀態未完全恢復到 before。這說明 user-space allocation primitive 可以對 kernel heap layout 產生可觀察影響，支撐 allocator massaging 與 memory reuse 的基本前提。
```
